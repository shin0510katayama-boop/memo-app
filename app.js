...
const COLORS = ["#00e5ff", "#7c5cff", "#39ff14", "#ffb020", "#ff4d6d", "#4da3ff"];
const els = {
...
function renderList() { const items = filtered(); els.listEmpty.classList.toggle("hidden", items.length > 0); els.list.innerHTML = items.map((m) => { const preview = m.body.replace(/\s+/g, " ").trim() || "EMPTY"; const active = m.id === state.selectedId ? "is-active" : ""; return `<li class="memo-item ${active}" data-id="${m.id}" style="border-left-color:${m.color}"><h3><span class="dot" style="background:${m.color}"></span>${escapeHtml(m.title || "UNTITLED")}${m.pinned ? '<span class="pin-mark">PIN</span>' : ""}</h3><p>${escapeHtml(preview)}</p></li>`; }).join(""); els.countAll.textContent = state.memos.filter((m) => !m.archived).length; els.countPinned.textContent = state.memos.filter((m) => m.pinned && !m.archived).length; els.countArchive.textContent = state.memos.filter((m) => m.archived).length; }
function renderEditor() { const memo = selected(); els.app.classList.toggle("is-editing", state.editing && !!memo); if (!memo) { els.editor.classList.add("hidden"); els.empty.classList.toggle("hidden", state.memos.length > 0); return; } els.empty.classList.add("hidden"); els.editor.classList.remove("hidden"); if (document.activeElement !== els.title && document.activeElement !== els.body) { els.title.value = memo.title; els.body.value = memo.body; } els.pin.textContent = memo.pinned ? "UNPIN" : "PIN"; els.archive.textContent = memo.archived ? "RESTORE" : "ARC"; els.updated.textContent = `UPD ${formatDate(memo.updatedAt)}`; els.chars.textContent = `${memo.body.length} CHARS`; renderColors(memo.color); }
